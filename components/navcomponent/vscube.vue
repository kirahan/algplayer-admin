<template>
  <v-container fluid style="padding:0px">
    ...
    <v-tabs-items v-model="tab">
      <v-tab-item key="One">
        <v-card tile dark elevation="0" color="teal lighten-1">
      ...
      <v-tab-item key="Two">
        <v-row>
          <v-expansion-panels accordion>
            <v-expansion-panel>
              <v-expansion-panel-header color="cyan darken-2 white--text"
                >魔方底色:{{ bottom }}</v-expansion-panel-header
              >
                ...
                <v-row no-gutters dark>
                  ...
                  >
                    <v-card
                      ...
                      @click="setbottomlayer(bottomlayer)"
                    >
                  ...
                  <v-col cols="12" class="bottombtn">
                    <v-btn @click="saveBottomLayer" color="success">确定</v-btn>
                    <v-btn @click="resetBottomLayer" color="error">重置</v-btn>
                  </v-col>
                </v-row>
              </v-expansion-panel-content>
            </v-expansion-panel>
            ...
            <v-expansion-panel>
              <v-expansion-panel-header color="purple darken-2 white--text"
                >配色模板</v-expansion-panel-header
              >
              <v-expansion-panel-content>
                <v-row no-gutters>
                  <v-col cols="2">
                  ...
                  <v-col cols="12" class="preset">
                    <span>预设</span>
                    <v-btn @click="saveThemeColors" small color="success"
                      >确定</v-btn
                    >
                    <v-btn @click="undoThemeColors" small color="warn"
                      >撤销</v-btn
                    >
                    <v-btn @click="resetThemeColors" small color="error"
                      >重置</v-btn
                    >
                  </v-col>
    ...
    <v-bottom-sheet dark inset v-model="colord">
      <v-card flat style="margin: auto; touch-action: none; user-select: none;">
        <v-container
          fluid
          grid-list-md
          text-xs-center
          :style="{ width: Math.min(size * 8, width) + 'px' }"
        >
          <v-layout row wrap>
            <v-flex
...
export default class Account extends Vue {
  ...
  bottomlist = [
    { name: "蓝", value: "blue", face: "B", color: "indigo" },
    { name: "绿", value: "green", face: "F", color: "green" },
    { name: "黄", value: "yellow", face: "U", color: "yellow" },
    { name: "白", value: "white", face: "D", color: "grey lighten-1" },
    { name: "橙", value: "orange", face: "R", color: "orange" },
    { name: "红", value: "red", face: "L", color: "red" }
  ];
  colorlist = [
    { name: "蓝", value: "blue", face: "B", color: "indigo" },
    { name: "绿", value: "green", face: "F", color: "green" },
  ...
  setthemecolor(data) {
    this.$sampleRender.setThemeColors(data);
    return data;
  }

  undoThemeColors() {
    this.$sampleRender.undoThemeColors();
  ...
  resetThemeColors() {
    this.$sampleRender.resetThemeColors();
    this.getlocalstoragecolors();
    this.snackbar = true;
    this.message = `重置成功`;
    return true;
  ...
  saveThemeColors() {
    this.$sampleRender.saveThemeColors();
    this.snackbar = true;
    this.message = `设置颜色成功`;
    return true;
  }

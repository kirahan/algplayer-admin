<template>
  <v-container fluid style="padding:0px">
    <v-row class="text-center">
      <v-col cols="6">
        <v-sheet tile elevation-10 color="red" class="pa-3">
          <div class="headline">{{ gcase }}</div>
          <span class="caption">Cases</span>
        </v-sheet>
      </v-col>
      <v-col cols="6">
        <v-sheet tile elevation-10 color="indigo" class="pa-3">
      ...
      <v-col cols="6">
        <v-sheet tile elevation-10 color="purple" class="pa-3">
      ...
      <v-col cols="6">
        <v-sheet tile elevation-10 color="orange" class="pa-3">
          <div class="headline">{{ gmau }}</div>
          <span class="caption">MAU</span>
        </v-sheet>
      </v-col>
      <v-col cols="12">
        <span class="caption grey--text text--lighten-3"
          >更新于: {{ updateAt }}</span
        >
      </v-col>
    </v-row>
...
<script lang="ts">
import {Vue, Component, Prop, Ref, Provide,Watch } from "vue-property-decorator";
...
export default class Statistic extends Vue {
  ...
  async getdata() {
    const res = await this.$axios.get(`http://solarsunrise.cn/api/cubenode/algdb/stats`);
